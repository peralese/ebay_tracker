...
def _select_columns(conn: sqlite3.Connection, table: str):
    cols = [r[1] for r in conn.execute(f"PRAGMA table_info({table})")]  # (cid, name, type, ...)
    # Prefer these if present; otherwise just select all columns
    preferred = ["id","sku","title","status","list_price","sold_price","ebay_item_id","itemId","item_id"]
...
def get_local_items():
    ...
    if dbp.exists():
        ...
        try:
            if not _table_exists(conn, _EBT_SQLITE_TABLE):
                return []
